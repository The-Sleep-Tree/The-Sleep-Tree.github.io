...
function loadVue() {
	...
	Vue.component('main-s2-1-display', {
		props: ['layer', 'data'],
		template: `
		<div><span v-if="player['S2'].log.points.lt('1e1000')">你有 </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}">{{formatSmall(player["S2"].log.log)}}</h2> 对数 ({{format(player["S2"].log.points)}} 常数)<span v-if="layers[layer].effectDescription"></span></span></div>
		`
	})
	Vue.component('main-s2-2-display', {
		props: ['layer', 'data'],
		template: `
		<div><span v-if="player['S2'].antilog.points.lt('1e1000')">你有 </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}">{{formatSmall(player["S2"].antilog.log)}}</h2> 错数 ({{format(player["S2"].antilog.points)}} 异数)<span v-if="layers[layer].effectDescription"></span></span></div>
		`
	})
